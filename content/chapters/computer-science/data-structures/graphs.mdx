...
<Quiz title="Graph Networks — quick check">
  ...
  <QuizQuestion question="True or false: a stack can be used to check whether brackets in an expression are balanced." options={["False", "Cannot be determined from the information given", "Sometimes true", "True — push each opening bracket and pop on each closing one; the last opened must be the first closed, which is exactly LIFO."]} answer={3} explanation="True — push each opening bracket and pop on each closing one; the last opened must be the first closed, which is exactly LIFO." />
</Quiz>
