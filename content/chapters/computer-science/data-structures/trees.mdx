...
<Quiz title="Tree Structures — quick check">
  ...
  <QuizQuestion question="True or false: a stack can be used to check whether brackets in an expression are balanced." options={["3 — it goes left of root 5, and then 1 goes left of 3.", "A stack — each new page is pushed, and Back pops the most recent page, which is last-in, first-out.", "The values in ascending sorted order.", "True — push each opening bracket and pop on each closing one; the last opened must be the first closed, which is exactly LIFO."]} answer={3} explanation="True — push each opening bracket and pop on each closing one; the last opened must be the first closed, which is exactly LIFO." />
</Quiz>
