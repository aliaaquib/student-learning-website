...
<Quiz title="Pattern Recognition — quick check">
  ...
  <QuizQuestion question="True or false: computational thinking can only be used with computers." options={["It is a huge problem hiding dozens of sub-problems — graphics, controls, scoring — which must be decomposed and tackled one at a time.", "A washing machine running its wash cycle, or a central heating timer — both follow predefined steps without human input each time.", "Abstraction — irrelevant detail (bus colour) is stripped away, leaving what matters (routes and times).", "False — it is a general problem-solving mindset that works for planning, studying and organising anything."]} answer={3} explanation="False — it is a general problem-solving mindset that works for planning, studying and organising anything." />
</Quiz>
