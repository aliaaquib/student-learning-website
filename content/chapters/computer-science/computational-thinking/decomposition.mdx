...
<Quiz title="Breaking Problems Down — quick check">
  ...
  <QuizQuestion question="Give an example of automation in everyday life." options={["Questions sharing the same structure can be solved with the same method, so spotting the pattern lets you reuse a method instead of starting from scratch.", "It is a huge problem hiding dozens of sub-problems — graphics, controls, scoring — which must be decomposed and tackled one at a time.", "False — it is a general problem-solving mindset that works for planning, studying and organising anything.", "A washing machine running its wash cycle, or a central heating timer — both follow predefined steps without human input each time."]} answer={3} explanation="A washing machine running its wash cycle, or a central heating timer — both follow predefined steps without human input each time." />
</Quiz>
