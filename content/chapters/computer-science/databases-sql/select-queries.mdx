...
<Quiz title="SELECT Queries — quick check">
  ...
  <QuizQuestion question="What is wrong with DELETE FROM Students; and how do you fix it?" options={["A foreign key — it stores the primary key of a row in the Customers table, linking each order to its customer.", "It deletes every row in the table — add a WHERE clause, for example DELETE FROM Students WHERE ID = 4;, to remove only Leo.", "SELECT Title, Author FROM Books WHERE Year = 2010;", "One row: Leo — only Leo has Year 10."]} answer={1} explanation="It deletes every row in the table — add a WHERE clause, for example DELETE FROM Students WHERE ID = 4;, to remove only Leo." />
</Quiz>
